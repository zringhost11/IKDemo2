...
export class PersonScript extends Laya.Script {
    ...
    onUpdate(): void {
        if (!this.leftChain) {
            this.leftChain = this.ikcom.getChain("mixamorig:LeftToeBase");
        }
        if (!this.rightChain) {
            this.rightChain = this.ikcom.getChain("mixamorig:RightToeBase");
        }
        let playName = "idle";
    ...
    private updateFootIK(toeBase: Laya.Sprite3D, chain: Laya.IK_Chain, isRightFoot: boolean): void {
        ...
        // 从更高的位置发射，确保从角色外部发射
        // CharacterController 的高度通常是 2 米左右，我们从脚部上方 0.5 米发射
